...
where g.reknr = 810000 and g.bkjrcode = ? and g.periode = ? and g.project is not NULL and g.bkstnr is not NULL and g.project != 'OPAKOWANIA' and artcode like '2%'
order by g.project"""
...
print(
    "project\tartcode\tdoc_number\tfulfill_value\tcostprice\tpieces\tdiff [%]".expandtabs(30))
...
for r in records:
    ...
    if (abs(diff) < 98 or abs(diff) > 102) and r.aantal != 0:
        print(f"{r.project}\t{r.artcode}\t{r.bkstnr}\t{r.bdr_hfl}\t{
              r.CostPriceStandard}\t{round(r.aantal)}\t{diff}%".expandtabs(30))
...
if update == 'y':
    ...
    for i in range(lists_len):
        cursor.execute(UPDATE_QUERY_GBKMUT, -abs(costprice_list[i]), -abs(
            costprice_list[i]), '   302000', int(docnumber_list[i]), artcode_list[i])
        cursor.execute(UPDATE_QUERY_GBKMUT, costprice_list[i], costprice_list[i], '   810000', int(
            docnumber_list[i]), artcode_list[i])

        cursor.execute(UPDATE_QUERY_AMUTAS, -abs(costprice_list[i]), -abs(
            costprice_list[i]), '   302000', int(docnumber_list[i]), artcode_list[i])
        cursor.execute(UPDATE_QUERY_AMUTAS, costprice_list[i], costprice_list[i], '   810000', int(
            docnumber_list[i]), artcode_list[i])
    conn.commit()
